...
- CONFIG的内容：
  ...
  | MIN_STEP                | 最小步数；多账号时可填写单个全局值，或使用 `#` 按账号顺序分别配置                                                                                  |
  | MAX_STEP                | 最大步数；多账号时可填写单个全局值，或使用 `#` 按账号顺序分别配置。步数随机范围随着时间线性增加，北京时间22点达到最大值                                              |
...
- 多账户请用 **#** 分割，然后保存到变量 **USER** 和 **PWD**；**MIN_STEP** 和 **MAX_STEP** 也可以用 **#** 按账号顺序配置独立范围
...
`MIN_STEP` 和 `MAX_STEP` 只有一个值时会应用于全部账号；使用 `#` 时，数量必须与账号数量一致。
